...
import { ActivityIndicator, Animated, Dimensions, Image, ImageBackground, Keyboard, KeyboardAvoidingView, Platform, TouchableOpacity, useColorScheme, View } from "react-native";
    ...
    if (nextUser) {
      ...
      try {
        ...
        if (fetched?.length > 0) {
          ...
          const nextUserData = await getUserByUid(nextUserId);
...
const getNextUserIdFromBackend = async (currentUserId: string): Promise<string | null> => {
  try {
    ...
    const currentIndex = allUsers.findIndex(user => user.id === currentUserId);
